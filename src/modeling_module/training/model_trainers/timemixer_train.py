"""Point-only trainer integration for the endogenous TimeMixer artifact."""
...
def train_timemixer(
    ...
    *,
    ...
    """Train TimeMixer through the shared trainer without changing its backbone."""

    assert train_cfg is not None, "train_cfg is required."
    if bool(getattr(train_cfg, "use_exogenous_mode", False)):
        raise RuntimeError("[train_timemixer] TimeMixer supports endogenous inputs only.")

    loss_mode = infer_loss_mode(train_cfg)
    if loss_mode != "point":
        raise NotImplementedError(
            "[train_timemixer] TimeMixer supports only point loss, "
    ...
    for index, stage in enumerate(stages, 1):
        ...
        print(f"\n[train_timemixer] ===== Stage {index}/{len(stages)} =====")
        ...
        dump_cfg(cfg_i, name="timemixer_train")
