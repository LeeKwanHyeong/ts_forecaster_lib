...
from typing import Tuple, Optional, Callable, Sequence, Dict, List
...
from model_test.future_exo_utils import build_lookup_tuple, build_week_index, make_part_future_exo_fn
from model_test.model_test_utils import make_forecast_result_table, add_week, calc_accuracy
...
from modeling_module.models import build_patchTST, build_patchTST_quantile
...
def load_plant_df(parquet_path: Path) -> pl.DataFrame:
    ...
    if 'demand_cumsum' not in df.columns:
        raise ValueError(
            ...
            "Current preprocessing depends on demand_cumsum-based demand_log_ago generation."
        )

    if 'warranty' not in df.columns:
        raise ValueError(
            f"'warranty' column is missing: {parquet_path}. "
            "Current preprocessing depends on warranty-based demand_log_ago generation."
        )
...
def resolve_experiment_objects(
    ...
    return ExperimentObjects(
        ...
        past_exo_cat_cols = PAST_EXO_CAT_COLS if use_past_exo else tuple(),
    )
...
def run_training(
    ...
    logger.info(
        "[TRAIN] start | plant=%s case=%s batch=%s seed=%s",
        cfg.plant,
        cfg.case_name,
        cfg.batch_size,
        cfg.seed,
    ...
    run_total_train_weekly(
        ...
        use_past_exogenous = cfg.use_past_exo,
        use_future_exogenous=cfg.use_future_exo,
        loss = LOSS,
...
def load_base_model(checkpoints_dir: Path):
    builders = {'patchtst_base': build_patchTST}
    model_dict = load_model_dict(str(checkpoints_dir), builders, device = DEVICE)
    if 'patchtst_base' not in model_dict:
        raise ValueError(
            f"'patchtst_base' checkpoint not found. checkpoints_dir={checkpoints_dir}"
        )
    return model_dict['patchtst_base']

def load_models(checkpoints_dir: Path):
    builders = {
        'patchtst_base': build_patchTST,
        'patchtst_quantile': build_patchTST_quantile,
    }
    model_dict = load_model_dict(str(checkpoints_dir), builders, device = DEVICE)
    base_model = model_dict.get('patchtst_base')
    quantile_model = model_dict.get('patchtst_quantile')

    if base_model is None:
        raise ValueError(f"'patchtst_base' checkpoint not found. checkpoints_dir={checkpoints_dir}")

    if quantile_model is None:
        raise ValueError(f"'patchtst_quantile' checkpoint not found. checkpoints_dir={checkpoints_dir}")
...
def infer_one_plan(
    ...
    inference_loader = data_module.get_inference_loader_at_plan(plan_week)
    ...
    logger.info("[INFER] saved plan_week=%s -> %s", plan_week, raw_result_path)
    return final_df

def load_plan_forecast(forecast_dir: Path, plan_weeks: Sequence[int]) -> Dict[int, pl.DataFrame]:
...
def build_revision_panel(
    ...
    for rev_idx, delta in enumerate(range(revisions - 1, -1, -1), start = 1):
        ...
        if src_plan_week not in forecast_dfs:
            raise KeyError(
                f"Failed to build revision panel: no result found for plan_week={src_plan_week}"
            )
    ...
    if base_df is None:
        raise ValueError(f"Cannot build revision panel for target_week={target_week}")
...
def aggregate_far_from_accuracy(df_acc: pl.DataFrame) -> pl.DataFrame:
    ...
    if not forecast_cols:
        raise ValueError("Missing '_base_forecast' column required for FAR aggregation.")
    if "accu" not in df_acc.columns:
        raise ValueError("Missing 'accu' column in calc_accuracy result.")
    if "fcst_qty" not in df_acc.columns:
        raise ValueError("Missing 'fcst_qty' column in calc_accuracy result.")
...
def evaluate_far(
    ...
    logger.info("[EVAL] FAR evaluation complete")
...
def save_sample_lineplots_for_plan_week_long_3col(
    ...
    plot_df = (
        final_df
        .filter(pl.col(id_col).is_in(sampled_parts))
    ...
    fig.suptitle(
        f"Plan Week {plan_week} | Sampled Parts Actual vs Forecast",
        fontsize = 14
    )
...
def sample_parts_for_plot(
    ...
    if len(part_list) == 0:
        raise ValueError("No candidates parts found for plotting")
...
def run_single_experiment(plant: str, case_name: str, batch_size: int, seed: int) -> None:
    ...
    full_df = load_plant_df(PLANT_REQUEST_MAP[plant])
    ...
    logger.info(
        "data ready | full_rows=%s train_rows=%s unique_parts=%s",
        full_df.height,
        train_df.height,
    ...
    base_model, quantile_model = load_models(paths.checkpoints_dir)
    ...
    forecast_dfs = load_plan_forecast(paths.forecast_dir, cfg.plan_weeks)
...
def main() -> None:
    ...
    for plant in PLANT_REQUEST_MAP:
