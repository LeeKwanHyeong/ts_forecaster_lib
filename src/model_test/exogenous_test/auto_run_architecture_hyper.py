...
from model_test.exogenous_test.auto_run_architecture_hyper_plot_utils import (
    plot_far_mean_base_ab_case_comparisons,
    plot_global_far_by_batch_case,
)
...
BASE_DIR = Path("E:/V101_future_exogenous_ab_test")
PLAN_WEEKS = list(range(202538, 202553)) + list(range(202601, 202615))
BATCH_SIZES = [
    # 256,
    # 128,
    64
...
PLANT_PARQUET_MAP = {
    "V101": Path('E:/in_out_wty_preprocessed/V101_in_out_wty.parquet'),
    'V101_Slice': Path('E:/in_out_wty_preprocessed/V101_slice_200_seq.parquet'),
    'V101_Concat': Path('E:/in_out_wty_preprocessed/concat_V101_200_V506_300.parquet')
}
        ...
        if bool(spec["use_future_exo"]):
            for fusion_mode in PATCHTST_FUTURE_EXO_FUSION_MODES:
                cases[f"{case_name}_{fusion_mode}"] = {
                    **spec,
                    "ab_case_name": f"{case_name}_{fusion_mode}",
                    "patchtst_future_exo_fusion_mode": fusion_mode,
                }
        else:
            cases[case_name] = {
                **spec,
                "ab_case_name": case_name,
                "patchtst_future_exo_fusion_mode": DEFAULT_PATCHTST_FUTURE_EXO_FUSION_MODE,
            }
    return cases


def build_experiment_cases() -> Dict[str, Dict[str, object]]:
    ab_cases = build_ab_cases()
    cases: Dict[str, Dict[str, object]] = {}
    for ab_case_name, ab_spec in ab_cases.items():
        for arch_case_name, arch_spec in PATCHTST_ARCH_CASES.items():
            case_key = f"{ab_case_name}__{arch_case_name}"
            cases[case_key] = {
                **ab_spec,
                **arch_spec,
                "arch_case_name": arch_case_name,
                "case_name": case_key,
            }
    return cases
...
def run_training(
    ...
    logger.info(
        "[TRAIN] start | plant=%s case=%s ab_case=%s arch_case=%s batch=%s seed=%s "
        "use_past_exo=%s use_future_exo=%s future_fusion=%s run_quantile=%s "
        "d_model=%s n_layers=%s d_ff=%s",
        cfg.plant,
        cfg.case_name,
        cfg.ab_case_name,
        cfg.arch_case_name,
...
def postprocess_forecast_df(
    ...
    forecast_df = result_df.select(
        [
            ...
            if c in {"oper_part_no", "plan_week", "forecast_week", "base_forecast", "quantile_forecast"}
        ]
    ).rename(rename_map)
...
def summarize_all_runs(base_dir: Path) -> None:
    ...
    grouped = (
        summary_df.group_by(
            ["plant", "ab_case_name", "arch_case_name", "batch_size", "d_model", "n_layers", "d_ff"]
        )
        .agg(
            [
        ...
        .sort(["plant", "ab_case_name", "arch_case_name", "batch_size"])
    )
    ...
    grouped = grouped.with_columns(
        (
            pl.col("ab_case_name")
            + pl.lit("__")
            + pl.col("arch_case_name")
            + pl.lit("__d")
            + pl.col("d_model").cast(pl.Utf8)
            + pl.lit("_l")
            + pl.col("n_layers").cast(pl.Utf8)
            + pl.lit("_ff")
            + pl.col("d_ff").cast(pl.Utf8)
        ).alias("case_label")
    )

    plot_global_far_by_batch_case(grouped, global_plots_dir)
    plot_far_mean_base_ab_case_comparisons(
        summary_df=summary_df,
        save_dir=global_plots_dir / "base_ab_case_far_mean",
        base_ab_case_names=list(BASE_AB_CASES.keys()),
        plant_order=list(PLANT_PARQUET_MAP.keys()),
    )
...
def run_single_experiment(plant: str, case_name: str, batch_size: int, seed: int) -> None:
    case_spec = EXPERIMENT_CASES[case_name]

    cfg = RunConfig(
        plant=plant,
        case_name=case_name,
        ab_case_name=str(case_spec["ab_case_name"]),
        arch_case_name=str(case_spec["arch_case_name"]),
        use_past_exo=bool(case_spec["use_past_exo"]),
        use_future_exo=bool(case_spec["use_future_exo"]),
        batch_size=batch_size,
        seed=seed,
        d_model=int(case_spec["d_model"]),
        n_layers=int(case_spec["n_layers"]),
        d_ff=int(case_spec["d_ff"]),
        run_patchtst_quantile=RUN_PATCHTST_QUANTILE,
        patchtst_future_exo_fusion_mode=str(case_spec["patchtst_future_exo_fusion_mode"]),
    )
...
def main() -> None:
    ...
    for plant in PLANT_PARQUET_MAP:
        for case_name in EXPERIMENT_CASES:
